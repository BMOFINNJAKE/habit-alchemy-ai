...
// Categories, colors, reminders, and recurrence options are the same as in add-block-dialog.tsx
...
export default function EditBlockDialog({
  ...
  const [reminderEnabled, setReminderEnabled] = useState(!!block.reminder)
  const [recurrenceEnabled, setRecurrenceEnabled] = useState(!!block.recurrence)
  const [formState, setFormState] = useState<TimeBlock>(block)

  // Update form state when block changes
  useEffect(() => {
    setFormState(block)
    setReminderEnabled(!!block.reminder)
    setRecurrenceEnabled(!!block.recurrence)
  }, [block])
  ...
  const updateFormState = (key: keyof TimeBlock, value: any) => {
    setFormState((prev) => ({
      ...prev,
      [key]: value,
    }))
          ...
          {showAdvanced && (
            <>
              <div className="grid gap-2">
                ...
                {reminderEnabled && (
                  <Select
                    value={(formState.reminder || 15).toString()}
